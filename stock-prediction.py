...
# df = pd.read_csv('./NSE-TATA.csv')
# df = df.iloc[::-1]
df = yf.download(tickers='BTC-USD', period='2d', interval='1m')
...
fig.add_trace(go.Scatter(x=df.index, y =df['Predictions'], line=dict(color='red'), name='prediction'))

fig3 = go.Figure(data = [go.Scatter(x=df.index, y =df['Close'], line=dict(color='blue'), name='Close')])
fig3.add_trace(go.Scatter(x=df.index, y =df['Predictions'], line=dict(color='red'), name='prediction'))
# end
